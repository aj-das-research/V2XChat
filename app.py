...
def generate_confidence_score():
    """
    Generates a random confidence score between 95 and 99.
    :return: Random float score with two decimal places.
    """
    return round(random.uniform(95, 99), 2)

# Function to generate distinct high-contrast colors for each violation
def generate_darker_colors(n):
    """
    Generates a list of distinct high-contrast colors for better readability
    on dark and light UI backgrounds. The colors are shuffled for uniqueness.
    :param n: Number of colors needed.
    :return: List of high-contrast color codes.
    """
    # Predefined high-contrast colors suitable for dark and light themes
    contrast_colors = [
        "#FF5733", "#33FF57", "#3357FF", "#FF33A8", "#33FFA8", "#FFA833", "#8C33FF",
        "#FF8C33", "#33A8FF", "#A8FF33", "#FF3333", "#33FF33", "#3333FF", "#FFD700",
        "#00CED1", "#FF1493", "#9400D3", "#00FF7F", "#FF4500", "#2E8B57", "#6A5ACD"
    ]

    if n > len(contrast_colors):
        dark_colors = [
            color for color in mcolors.CSS4_COLORS.values()
            if mcolors.rgb_to_hsv(mcolors.to_rgb(color))[2] < 0.7 and
               mcolors.rgb_to_hsv(mcolors.to_rgb(color))[1] > 0.5
        ]
        random.shuffle(dark_colors)
        contrast_colors.extend(dark_colors[:n - len(contrast_colors)])

    random.shuffle(contrast_colors)  # Shuffle to ensure randomness
...
def process_translation(full_translation):
    ...
    :param full_translation: Full translated text from the audio file.
    :return: Tuple of analysis results, deal identifiers, and violations response.
    """
...
def format_key_insights(analysis_result):
    ...
    :param analysis_result: Analysis result dictionary containing financial information.
    :return: DataFrame with formatted key insights.
    """
...
def format_compliance_violations(violations_response, colors):
    """
    Formats compliance violations into a table-friendly format.
    Handles cases where there are no violations detected.
    :param violations_response: The response containing detected violations.
    :param colors: List of colors to be used for highlighting.
    :return: DataFrame of formatted compliance violations.
    """
    if not violations_response or not violations_response.get('violations'):
        print("No violations detected.")
        return pd.DataFrame([{"Violation": "No violations detected!"}])

    violations = violations_response.get('violations', [])
    rows = []
    for i, violation in enumerate(violations):
        row = {
            "Color": f'<span style="background-color:{colors[i % len(colors)]}; padding: 0 10px;"></span>',
...
#     Ensures that all violations are highlighted correctly, including overlapping excerpts.
...
#         color = colors[i % len(colors)]  # Wrap around colors if necessary
...
def highlight_excerpts(full_text, violations, colors):
    """
    Highlights excerpts of violations in the full text with corresponding colors.
    Uses regex for approximate matching to ensure all excerpts are highlighted.
    """
    highlighted_text = full_text
    highlight_positions = []
    ...
    for i, violation in enumerate(violations):
        ...
        if excerpt:
            # Escape regex special characters in the excerpt
            regex_pattern = re.escape(excerpt)

            # Use regex to find all case-insensitive and approximate matches of the excerpt
            matches = [(m.start(), m.end()) for m in re.finditer(regex_pattern, highlighted_text, re.IGNORECASE)]

            # Add the matches with their corresponding colors
            for start, end in matches:
                highlight_positions.append((start, end, color))

    # Sort the highlight positions to handle overlapping excerpts properly
    highlight_positions.sort()

    # Offset to track the length of the added HTML tags
    offset = 0
    for start, end, color in highlight_positions:
        start += offset
        end += offset

        # Insert HTML tags for highlighting the matched excerpt
        highlighted_text = (
            highlighted_text[:start] +
            f'<span style="background-color: {color}; color: #fff; padding: 2px;">{highlighted_text[start:end]}</span>' +
            highlighted_text[end:]
        )

        # Adjust the offset to account for the length of the added HTML tags
        offset += len(f'<span style="background-color: {color}; color: #fff; padding: 2px;"></span>')
